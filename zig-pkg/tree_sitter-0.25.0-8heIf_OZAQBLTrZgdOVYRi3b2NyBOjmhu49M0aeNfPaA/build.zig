...
pub fn build(b: *std.Build) !void {
    ...
    // HACK: fetch tree-sitter-c only when running `zig build test`
    //       (original motivation: ziglang/zig#19914)
    if (b.pkg_hash.len > 0) return;

    const cmd_args = b.args orelse return;  // b.args == null → not a command like "test"
